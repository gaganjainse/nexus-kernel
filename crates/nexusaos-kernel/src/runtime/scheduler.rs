...
mod tests {
    ...
    async fn test_enqueue_dequeue() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let entry = scheduler.dequeue().await.ok_or("queue should not be empty")?;
    ...
    async fn test_priority_ordering() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(scheduler.dequeue().await.ok_or("queue should not be empty")?.task_id, task2);
        assert_eq!(scheduler.dequeue().await.ok_or("queue should not be empty")?.task_id, task3);
        assert_eq!(scheduler.dequeue().await.ok_or("queue should not be empty")?.task_id, task1);
        Ok(())
    }
    ...
    async fn test_queue_depth_after_dequeue() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let _ = scheduler.enqueue(TaskId::new(), Priority::Normal).await?;
        assert_eq!(scheduler.queue_depth(), 1);
        let _ = scheduler.dequeue().await.ok_or("queue should not be empty")?;
        assert_eq!(scheduler.queue_depth(), 0);
        Ok(())
    }
    ...
    async fn test_enqueue_dequeue_critical_priority() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(scheduler.dequeue().await.ok_or("queue should not be empty")?.task_id, crit_id);
        Ok(())
    }
    ...
    async fn test_fifo_within_same_priority() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(scheduler.dequeue().await.ok_or("queue should not be empty")?.task_id, t1);
        assert_eq!(scheduler.dequeue().await.ok_or("queue should not be empty")?.task_id, t2);
        Ok(())
    }
    ...
    async fn test_enqueue_dequeue_mixed_priorities() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let first = scheduler.dequeue().await.ok_or("queue should not be empty")?;
        assert_eq!(first.priority, Priority::Critical);
        for _ in 0..3 {
            assert_eq!(
                scheduler.dequeue().await.ok_or("queue should not be empty")?.priority,
                Priority::High
            );
        }
        for _ in 0..5 {
    ...
    async fn test_cancel_middle_of_queue() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let second = scheduler.dequeue().await.ok_or("queue should not be empty")?;
