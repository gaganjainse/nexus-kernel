...
"#
    ...
    #[test]
    fn test_load_from_string() -> Result<(), Box<dyn std::error::Error>> {
        let config = AppConfig::parse_toml(sample_toml())?;
...
"#;
    ...
    #[test]
    fn test_resolved_data_dir_absolute() -> Result<(), Box<dyn std::error::Error>> {
        let config = AppConfig::parse_toml(sample_toml())?;
    ...
    #[test]
    fn test_resolved_data_dir_tilde() -> Result<(), Box<dyn std::error::Error>> {
        let toml = sample_toml().replace("/tmp/nexusaos-test", "~/.nexusaos");
        let config = AppConfig::parse_toml(&toml)?;
    ...
    #[test]
    fn test_default_tool_config() -> Result<(), Box<dyn std::error::Error>> {
        let config = AppConfig::parse_toml(sample_toml())?;
    ...
    #[test]
    fn test_validation_zero_max_queue_depth() -> Result<(), Box<dyn std::error::Error>> {
        let toml = r#"
[general]
data_dir = "/tmp/test"
[resource_limits]
...
"#;
        ...
        let Err(err) = result else {
            return Err("expected parse_toml to reject max_queue_depth = 0".into());
        };
    ...
    #[test]
    fn test_validation_zero_max_context_tokens() -> Result<(), Box<dyn std::error::Error>> {
        let toml = r#"
[general]
data_dir = "/tmp/test"
[resource_limits]
...
"#;
        let result = AppConfig::parse_toml(toml);
        let Err(err) = result else {
            return Err("expected parse_toml to reject max_context_tokens = 0".into());
        };
    ...
    fn test_from_str() -> Result<(), Box<dyn std::error::Error>> {
        let config: AppConfig = sample_toml().parse()?;
        assert_eq!(config.general.data_dir, "/tmp/nexusaos-test");
        Ok(())
    }
    ...
    fn test_load_from_file() -> Result<(), Box<dyn std::error::Error>> {
        ...
        std::fs::write(&config_path, sample_toml())?;
    ...
    #[test]
    fn test_load_missing_file() -> Result<(), Box<dyn std::error::Error>> {
        let Err(err) = AppConfig::load("/nonexistent/path/config.toml") else {
            return Err("expected loading a missing config file to fail".into());
        };
        match err {
            ConfigError::NotFound { .. } => {}
            other => return Err(format!("expected NotFound error, got {other:?}").into()),
        }
        Ok(())
    }
    ...
    #[test]
    fn test_multiple_model_providers() -> Result<(), Box<dyn std::error::Error>> {
        let toml = r#"
[general]
data_dir = "/tmp/test"
[resource_limits]
...
"#;
    ...
    #[test]
    fn test_default_log_level() -> Result<(), Box<dyn std::error::Error>> {
        let toml = r#"
[general]
data_dir = "/tmp/test"
[resource_limits]
...
"#;
    ...
    #[test]
    fn test_default_policy_values() -> Result<(), Box<dyn std::error::Error>> {
        let toml = r#"
[general]
data_dir = "/tmp/test"
[resource_limits]
...
"#;
    ...
    #[test]
    fn test_default_shutdown_config() -> Result<(), Box<dyn std::error::Error>> {
        let toml = r#"
[general]
data_dir = "/tmp/test"
[resource_limits]
...
"#;
    ...
    #[test]
    fn test_default_tools_config() -> Result<(), Box<dyn std::error::Error>> {
        let toml = r#"
[general]
data_dir = "/tmp/test"
[resource_limits]
...
"#;
    ...
    #[test]
    fn test_resolved_data_dir_nonexistent_home() -> Result<(), Box<dyn std::error::Error>> {
...
"#;
    ...
    #[test]
    fn test_model_provider_config_serde() -> Result<(), Box<dyn std::error::Error>> {
        let toml = r#"
name = "test"
role = "vision"
...
"#;
        let config: ModelProviderConfig = toml::from_str(toml)?;
    ...
    #[test]
    fn test_context_config_fields() -> Result<(), Box<dyn std::error::Error>> {
        let toml = r#"
[general]
data_dir = "/tmp/test"
[resource_limits]
...
"#;
    ...
    #[test]
    fn test_resource_limits_fields() -> Result<(), Box<dyn std::error::Error>> {
        let toml = r#"
[general]
data_dir = "/tmp/test"
[resource_limits]
