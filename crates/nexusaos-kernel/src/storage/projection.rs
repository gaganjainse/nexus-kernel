...
mod tests {
    ...
    fn test_projection_rebuild() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let task = projection.get_task(&task_id).ok_or("task should exist in projection")?;
    ...
    fn test_apply_task_created() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let task = proj.get_task(&task_id).ok_or("task should exist in projection")?;
    ...
    fn test_rebuild_multiple_tasks_different_states() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(
            proj.get_task(&t1).ok_or("task should exist in projection")?.current_state,
            TaskState::Completed
        );
        assert_eq!(
            proj.get_task(&t2).ok_or("task should exist in projection")?.current_state,
            TaskState::Received
        );
        Ok(())
    }
