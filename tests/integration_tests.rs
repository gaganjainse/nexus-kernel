...
async fn test_end_to_end_task_submission_and_state() {
    ...
    let kernel = Kernel::new(event_store.clone(), Arc::new(RwLock::new(policy)), registry, broker, 1_048_576, None, ResourceBudget::default(), Arc::new(ResourceMonitor), Arc::new(ContextManager::new(ContextConfig::default())), Arc::new(Scheduler::new(32)), 5).await.unwrap();
...
async fn test_policy_enforcement_denied_action() {
    ...
    let kernel = Kernel::new(event_store, Arc::new(RwLock::new(policy)), registry, broker, 1_048_576, None, ResourceBudget::default(), Arc::new(ResourceMonitor), Arc::new(ContextManager::new(ContextConfig::default())), Arc::new(Scheduler::new(32)), 5).await.unwrap();
